...
type ProviderId = "mercadopago" | "asaas" | "infinitepay";
type Convite = {
...
const PROVIDER_NOME: Record<ProviderId, string> = { mercadopago: "Mercado Pago", asaas: "Asaas", infinitepay: "InfinitePay" };
...
export default function PagamentoSelfService({ token, convite, sistema, plano, provider, mpPublicKey }: {
  token: string;
  convite: Convite;
  ...
  useEffect(() => {
    if (provider === "mercadopago" && mpPublicKey && (window as any).MercadoPago && !mpReady) setMpReady(true);
  }, [provider, mpPublicKey, mpReady]);
  ...
  async function enviarAoServidor(body: Record<string, any>) {
    ...
    try {
      ...
      if (j.redirect && j.checkoutUrl) { window.location.href = j.checkoutUrl; return; }
      setOk({ simulado: j.simulado });
    } catch { setErr("Falha de conexão. Tente de novo."); }
    setLoading(false);
  }

  async function confirmarInfinitePay() {
    await enviarAoServidor({});
  }

  async function confirmarAsaas() {
    ...
    if (soDigitos(cpfCnpj).length < 11) { setErr("Informe um CPF ou CNPJ válido."); return; }
    await enviarAoServidor({ cartao: { numero: soDigitos(cardNum), nome: cardNome, validade: cardVal, cvv: cardCvv, cpfCnpj: soDigitos(cpfCnpj) } });
  }

  async function confirmarMercadoPago() {
    if (soDigitos(cardNum).length < 13 || soDigitos(cardVal).length < 4 || soDigitos(cardCvv).length < 3 || !cardNome.trim()) {
      setErr("Confira os dados do cartão."); return;
    }
    if (!mpReady || !mpPublicKey) { setErr("Pagamento ainda carregando, tente novamente em alguns segundos."); return; }
    setErr(""); setLoading(true);
    try {
      const mp = new (window as any).MercadoPago(mpPublicKey, { locale: "pt-BR" });
      const [mes, ano] = cardVal.split("/");
      const tokenResp = await mp.createCardToken({
        cardNumber: soDigitos(cardNum), cardholderName: cardNome,
        cardExpirationMonth: mes, cardExpirationYear: ano?.length === 2 ? `20${ano}` : ano,
        securityCode: cardCvv,
      });
      if (!tokenResp?.id) { setErr("Cartão recusado pelo Mercado Pago. Confira os dados."); setLoading(false); return; }
      await enviarAoServidor({ mpCardToken: tokenResp.id });
    } catch (e: any) {
      setErr("Não foi possível validar o cartão com o Mercado Pago."); setLoading(false);
    }
  }
    ...
    return (
      <Shell cor={cor}>
        <div style={{ ...card, alignItems: "center", textAlign: "center" }}>
          ...
          {ok.simulado && <div style={{ fontSize: 11.5, color: "var(--warn)", background: "color-mix(in srgb, var(--warn) 12%, transparent)", border: "1px solid color-mix(in srgb, var(--warn) 30%, transparent)", borderRadius: 9, padding: "9px 12px" }}>Modo simulado: nenhuma cobrança real foi feita ainda (chave do {PROVIDER_NOME[provider]} não configurada).</div>}
  ...
  return (
    <Shell cor={cor}>
      {provider === "mercadopago" && mpPublicKey && (
      ...
      <div style={card}>
        ...
        {provider === "infinitepay" ? (
          <>
            <div style={{ fontSize: 12.5, color: "var(--muted)" }}>Você vai ser levado(a) pra uma página segura da <b>InfinitePay</b> pra pagar com Pix ou cartão.</div>
            {err && <div style={{ color: "var(--crit)", fontSize: 12.5 }}>{err}</div>}
            <button type="button" disabled={loading} onClick={confirmarInfinitePay}
          ...
          <>
            ...
            {provider === "asaas" && (
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                ...
                <input style={inp} value={cpfCnpj} inputMode="numeric" onChange={(e) => setCpfCnpj(e.target.value)} placeholder="000.000.000-00" />
              </div>
            )}

            <div style={{ fontSize: 11.5, color: "var(--muted)", display: "flex", gap: 7, alignItems: "flex-start" }}>
              <span>🔒</span>
              <span>Protegido pelo <b>{PROVIDER_NOME[provider]}</b> — o número completo do cartão nunca fica guardado pela Perspecta.</span>
            </div>

            {err && <div style={{ color: "var(--crit)", fontSize: 12.5 }}>{err}</div>}

            <button type="button" disabled={loading} onClick={provider === "mercadopago" ? confirmarMercadoPago : confirmarAsaas}
              style={{ background: cor, color: "#fff", border: "none", borderRadius: 9, padding: "12px 16px", fontWeight: 650, fontSize: 14.5, cursor: "pointer", opacity: loading ? 0.7 : 1 }}>
