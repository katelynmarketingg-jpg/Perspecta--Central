...
export default function SimuladorPlanos({ sistemas, rateioPagando = 0 }: { sistemas: Sis[]; rateioPagando?: number }) {
  ...
  function adicionar() {
    if (preco <= 0) return;
    setLista((l) => [...l, { id: Date.now(), sistema, cor, nome: nome || "Plano", logins, gb, preco, custoHoje, custoPagando }]);
    setNome("");
  }
  ...
  return (
    <>
      <Card title="Simulador de plano" hint="custo hoje × quando pagar → lucro e markup">
        ...
        {/* Preço em destaque */}
        <div style={{ marginTop: 16, marginBottom: 4, fontSize: 12.5, color: "var(--muted)" }}>Vendendo por <b style={{ color: "var(--text)" }}>{BRL(preco)}/mês</b>, o resultado nos dois cenários:</div>

        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(240px,1fr))", gap: 12 }}>
          <Cenario titulo="Hoje (infra grátis)" custo={custoHoje} preco={preco} destaque={false} />
          <Cenario titulo="Quando começar a pagar" custo={custoPagando} preco={preco} destaque rateio={rateioPagando} />
        </div>
